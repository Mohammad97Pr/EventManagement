...
{
    public abstract class EventManagementWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static EventManagementWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(EventManagementWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<EventManagementDbContext> action)
        {
            using (var context = IocManager.Resolve<EventManagementDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<EventManagementDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<EventManagementDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<EventManagementDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<EventManagementDbContext, Task<T>> func)
        {
            T result;
